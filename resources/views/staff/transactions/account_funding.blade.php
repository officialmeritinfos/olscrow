                    ...
                    <table class="table align-middle mb-0">
                        ...
                        @foreach($transactions as $transaction)
                            <tr>
                                ...
                                <td>
                                    {{$transaction->currency}}{{number_format($transaction->amountToPay,2)}}
                                    ({{$transaction->currency}}{{number_format($transaction->amount,2)}})
                                </td>
                                <td>
                                    {{empty($transaction->bank)?'N/A':$transaction->bank}}
                                </td>
                                <td>
                                    {{empty($transaction->accountNumber)?'N/A':$transaction->accountNumber}}
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                            <li>
                                                <a class="dropdown-item" href="{{route('staff.transactions.funding.detail',['id'=>$transaction->reference])}}">
                                                    Details
